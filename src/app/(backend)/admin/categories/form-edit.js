...
export default function CategoriesFormEdit({
  open,
  onClose,
  onSuccess,
  category,
  ...
  useEffect(() => {
    category.name
      ...
      : form.setValue("name", "");
    category.name_dv
      ? form.setValue("name_dv", category.name_dv)
  ...
  async function onSubmit(values) {
    setIsLoading(true);
    const data = await updateCategory({ ...values, id: category.id });
    if (data) {
      onSuccess();
      setIsOpen(false);
    }
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <div className="flex flex-col h-full">
              <SheetHeader>
                ...
                <SheetDescription>
                  Fill out the form below to edit the category details. Please
